...
- The watch will try and delay sending a command until its last data poll is successful. It will display "Waiting for the vehicle to come back online" during that time that it waits.
- Added ? and ¿ as spinner symbols. These two indicate that an error was received by the watch instead of valid data.
- Added a W 'tag' to the spinner symbols. This tag is added when at least a command is in the buffer queue. If it stays there for a few seconds, it means the watch is having issues talking to the vehicle.
