...
/*DEBUG	if (_debug_view) {
	...
	function onReceiveToken(responseCode, data) {
		...
		if (responseCode == 200) {
			...
			//DEBUG*/ var expireAt = new Time.Moment(created_at + expires_in); var clockTime = Gregorian.info(expireAt, Time.FORMAT_MEDIUM); var dateStr = clockTime.hour + ":" + clockTime.min.format("%02d") + ":" + clockTime.sec.format("%02d");
